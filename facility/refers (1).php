          ...
          <?php
              ...
              if($move)
              {
                  if($user->refer($referred_by,$firstname,$middlename,$lastname,$gender,$telephone,$dob,$sickness,$priority,$diagnosis,$specialty,$cover_link))
                  {
                    ...
                    $postRefer_stmt = $user->runQuery("SELECT distance FROM temp_dist WHERE batch=:bat ORDER BY distance ASC LIMIT 2");
                    $postRefer_stmt->execute(array(":bat"=>date('Y-m-d')));
                    $k = $search_row['hospital_name'];
                    ...
                    function kNN($k, $trainingSet, $referringFacility) {
                        $distances = array();
                        $length = count($referringFacility);
                        
                        // Calculate Euclidean distance between referringFacility and the 3 closest facilities with the required Specialty
                        foreach ($trainingSet as $instance) {
                            $dist = euclideanDistance($referringFacility, $instance['features']);
                            $distances[] = array('distance' => $dist, 'class' => $instance['class']);
                        }
                    ...
                    while ($postRefer_row = $postRefer_stmt->fetch(PDO::FETCH_ASSOC)) {
                        $trainingSet = $postRefer_row['specialization'];
                        $destFacilities = $search_row['hospital'];

                        $prediction = kNN($k, $trainingSet, $destFacilities);

                        $searchTemp_stmt = $user->runQuery("SELECT * FROM temp_dist WHERE distance=:dist AND batch=:bat");
                        $searchTemp_stmt->execute(array(":dist"=>$trainingSet, ":bat"=>date('Y-m-d')));
                        ...
                        if ($postRefer_stmt) {
                            ...
                            $predict = $searchTemp_row['destination_hospital'];
                            if ($user->post_refer($referral_id,$predict)) {
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <?php
                ...
                <div class="container">
                    ...
                    <strong>ERROR!</strong> The Document must not e more than 1MB and filetype must be PDF !
                    </div>
                </div>
                <?php
                } else {}
